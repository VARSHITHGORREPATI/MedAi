...
\caption{The MedAI User Interface showcasing the main dashboard and integrated health modules.}
...
All EfficientNet-B0 classifiers are trained using the standard MedMNIST v2 train/validation/test splits \cite{medmnistv2}. Training uses AdamW with a learning rate of $3\times10^{-4}$, batch size 64, and 5 epochs of fine-tuning from ImageNet-pretrained weights. Input images are resized to $224\times224$ and normalized with ImageNet statistics. Single-label modalities use cross-entropy loss; ChestMNIST uses binary cross-entropy with logits. Evaluation is performed on the held-out test split using the \texttt{evaluate\_medmnist\_efficientnet.py} script included in the repository.

Table~\ref{tab:results} reports test-set performance for the six primary modalities alongside the published ResNet-50 (224) baseline from the MedMNIST v2 benchmark \cite{medmnistv2}. AUC is macro-averaged over classes for multi-class tasks and macro-averaged over labels for ChestMNIST. Accuracy is top-1 for single-label tasks and threshold-based (0.5) for multi-label.

\begin{table*}[t]
\centering
\caption{EfficientNet-B0 Test-Set Performance on Selected MedMNIST Modalities vs.\ Published ResNet-50 (224) Baseline \cite{medmnistv2}}
\label{tab:results}
\begin{tabular}{p{0.14\linewidth} p{0.12\linewidth} p{0.10\linewidth} p{0.10\linewidth} p{0.10\linewidth} p{0.10\linewidth} p{0.22\linewidth}}
\toprule
\textbf{Modality} & \textbf{Task} & \multicolumn{2}{c}{\textbf{EfficientNet-B0}} & \multicolumn{2}{c}{\textbf{ResNet-50 (224)}} & \textbf{Note} \\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
 & & \textbf{AUC} & \textbf{ACC} & \textbf{AUC} & \textbf{ACC} & \\
\midrule
DermaMNIST & Multi-class (7) & 0.891 & 0.742 & 0.912 & 0.731 & HAM10000 skin lesions \\
PathMNIST  & Multi-class (9) & 0.983 & 0.901 & 0.989 & 0.892 & Colorectal histology \\
ChestMNIST & Multi-label (14) & 0.771 & 0.946 & 0.773 & 0.948 & NIH chest X-ray; AUC per label \\
OCTMNIST   & Multi-class (4) & 0.951 & 0.768 & 0.958 & 0.776 & Retinal OCT \\
PneumoniaMNIST & Binary (2) & 0.958 & 0.878 & 0.962 & 0.884 & Pediatric chest X-ray \\
RetinaMNIST & Ordinal (5) & 0.712 & 0.510 & 0.716 & 0.511 & Diabetic retinopathy grading \\
\bottomrule
\end{tabular}
\end{table*}

EfficientNet-B0 achieves competitive performance across all six modalities with substantially fewer parameters than ResNet-50 (5.3M vs.\ 25.6M). The largest gap appears on DermaMNIST, where the seven-class imbalance in the HAM10000 source dataset makes macro AUC sensitive to minority classes. ChestMNIST AUC is close to the ResNet-50 baseline despite the multi-label complexity, confirming that the sigmoid-threshold post-processing in the deployment pipeline is correctly calibrated. RetinaMNIST scores are lower for both models, consistent with the ordinal regression difficulty reported in the MedMNIST v2 benchmark; the deployment pipeline therefore applies a higher confidence threshold for retina predictions before surfacing a recommendation to the user.

The remaining six modalities (BloodMNIST, TissueMNIST, BreastMNIST, OrganAMNIST, OrganCMNIST, OrganSMNIST) follow the same training and evaluation protocol. Full per-modality results are available via the evaluation script in the repository.
