...
function MessageBubble({ message }) {
  ...
  const handleAddReminders = async (diagnosis) => {
    try {
      ...
      const meds = diagnosis?.carePlan?.pharmacy_medicines || [];
      if (!meds.length) {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data?.success === false) {
        throw new Error(data?.detail || data?.message || 'Failed to add reminders');
      }
      setActionStatus(data?.message || 'Medicine reminders added.');
    } catch (err) {
  ...
  const createAppointmentRequest = async (diagnosis) => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data?.success === false) {
      throw new Error(data?.detail || data?.message || 'Appointment failed');
    }
  ...
  const createReminderRequest = async (diagnosis) => {
    ...
    const meds = diagnosis?.carePlan?.pharmacy_medicines || [];
    if (!meds.length) throw new Error('No medicines for reminders.');
    const medicines = meds.slice(0, 5).map((med) => ({
      name: med,
      dosage: 'As advised by clinician',
      frequency: 'twice daily',
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data?.success === false) {
      throw new Error(data?.detail || data?.message || 'Reminder creation failed');
    }
    return data?.message || 'Reminders added';
  };
  ...
  const createLabRequest = async (diagnosis) => {
    ...
    const address = labAddress || user?.location || '';
    const phone = labPhone || user?.mobile || '';
    if (!address || !phone) throw new Error('Lab address/phone missing.');
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data?.success === false) {
      throw new Error(data?.detail || data?.message || 'Lab booking failed');
    }
    return 'Lab tests booked';
  };
  ...
  const createPharmacyRequest = async (diagnosis) => {
    ...
    const items = selectedPharmacyItems.length
      ? selectedPharmacyItems
      : (diagnosis?.carePlan?.pharmacy_medicines || []).slice(0, 4).map((name, idx) => ({ key: `${name}-${idx}`, name, quantity: 1 }));
    if (!items.length) throw new Error('No pharmacy items available.');

    const orderItems = [];
    for (const med of items) {
      const res = await fetch(`${API_URL}/api/products?search=${encodeURIComponent(med.name)}&limit=1`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok && data?.success && Array.isArray(data.data) && data.data.length > 0) {
    ...
    const addr = pharmacyForm.address || user?.location || '';
    const city = pharmacyForm.city || 'NA';
    const pincode = pharmacyForm.pincode || '000000';
    const phone = pharmacyForm.phone || user?.mobile || '';
    if (!addr || !phone) throw new Error('Pharmacy address/phone missing.');
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data?.success === false) {
      throw new Error(data?.detail || data?.message || 'Pharmacy draft failed');
    }
    return data?.order_number ? `Pharmacy draft ${data.order_number}` : 'Pharmacy draft created';
  };
            ...
            <div className="space-y-4">
              ...
              {diagnosis.carePlan && (
                <>
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="rounded-xl bg-sidebar border border-sidebar-border p-3">
                      <p className="text-xs uppercase tracking-wide text-muted mb-2">MedAI Doctor Suggestions</p>
                      <ul className="space-y-2 text-sm text-white">
                        {(diagnosis.carePlan.doctor_suggestions || []).slice(0, 4).map((doctor, idx) => (
                          <li key={`${doctor.name || 'doctor'}-${idx}`} className="border-b border-sidebar-border/40 pb-2 last:border-none last:pb-0">
                            <p className="font-semibold">{doctor.name}</p>
                            <p className="text-muted text-xs">{doctor.specialty} - {doctor.role}</p>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="rounded-xl bg-sidebar border border-sidebar-border p-3">
                    <p className="text-xs uppercase tracking-wide text-muted mb-2">Chatbot Quick Actions</p>
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => handleCreateAllActions(diagnosis)}
                        disabled={createAllLoading}
                        className="px-3 py-1.5 text-xs rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:from-purple-700 hover:to-indigo-700 transition-colors disabled:opacity-50"
                      >
                        {createAllLoading ? 'Creating All...' : 'Create All Care Actions'}
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="px-3 py-1.5 text-xs rounded-lg bg-primary text-white hover:bg-blue-600 transition-colors"
                      >
                        Book Appointment
                      </button>
                      <button
                        ...
                        className="px-3 py-1.5 text-xs rounded-lg bg-blue-500/20 border border-blue-400/40 text-blue-200 hover:bg-blue-500/30 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-xs rounded-lg bg-cyan-500/20 border border-cyan-400/40 text-cyan-200 hover:bg-cyan-500/30 transition-colors"
                      >
                        Book Lab Tests
                      </button>
                      <button
                        ...
                        onClick={() => {
                          setActionStatus('');
                          handleOpenPharmacyDraft(diagnosis);
                        }}
                        className="px-3 py-1.5 text-xs rounded-lg bg-indigo-500/20 border border-indigo-400/40 text-indigo-200 hover:bg-indigo-500/30 transition-colors"
                      >
                        Create Pharmacy Draft
                      </button>
                      <button
                        type="button"
                        onClick={() => openModule('/health-tracking')}
                        className="px-3 py-1.5 text-xs rounded-lg bg-green-500/20 border border-green-400/40 text-green-200 hover:bg-green-500/30 transition-colors"
                      >
                        Health Tracking
                      </button>
                    </div>
                    {createAllProgress.length > 0 && (
                      <div className="mt-3 rounded-lg border border-sidebar-border bg-sidebar-hover p-2.5">
                        <p className="text-[11px] uppercase tracking-wide text-muted mb-2">Create All Progress</p>
                        <div className="space-y-1.5">
                          {createAllProgress.map((item, idx) => (
                            <div key={`${item.step}-${idx}`} className="flex items-start gap-2 text-xs">
                              <span className="mt-0.5">
                                {item.state === 'running' ? '⏳' : item.state === 'success' ? '✅' : '⚠️'}
                              </span>
                              <span className="text-white">
                                {item.step}: <span className="text-muted">{item.detail}</span>
                              </span>
                            </div>
                          ))}
                        </div>
                    ...
                    {actionStatus && (
                      <p className="text-xs text-cyan-200 mt-2 whitespace-pre-line">{actionStatus}</p>
                    )}
                  </div>
                  ...
                  {showBookingModal && (
                    <div className="rounded-xl border border-primary/40 bg-sidebar p-4">
                      ...
                      <div className="space-y-2 mb-3">
                        <p className="text-xs text-muted uppercase tracking-wide">Choose Doctor</p>
                        {(diagnosis.carePlan?.doctor_suggestions || []).slice(0, 6).map((doctor, idx) => (
                          <label
                          ...
                          >
                            ...
                            <span>
                              ...
                              <span className="block text-xs text-muted">{doctor.role}</span>
                            </span>
                          </label>
                        ))}
                      </div>
